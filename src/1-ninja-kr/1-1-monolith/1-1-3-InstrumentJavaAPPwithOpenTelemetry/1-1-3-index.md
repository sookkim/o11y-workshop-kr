...
- --with-instrumentation 옵션을 통해 제로코드 계측에 필요한 라이브러리를 다운로드 받았습니다
- --deployment-environment 옵션을 통해서 APM 에 태그로 동작 할 env 정보에 대해서 선언 했습니다.
- --enable-profiler --enable-profiler-memory --enable-metrics 옵션을 통해서 APM 의 확장기능인 프로파일링 기능을 활성화 하였습니다

제로 코드 계측 옵션을 선택했다는 것은, 위 설명드린 매뉴얼작업들을 자동화하여 번거롭게 일일이 설정하지 않아도 된다는 뜻이며, 소스코드나 Application의 실행스크립트를 변경하지 않고서도 APM 계측이 가능하게 하는 기능입니다.

</br>

### 설정파일 확인

아래 경로로 이동하여 JAVA 제로코드 옵션이 생성 된 것을 확인 합니다

```bash
cd /etc/splunk/zeroconfig/
...
앱실행 명령어를 보시면 아시겠지만, 펫클리닉을 빌드하고 실행시켰던 가장 처음 실행 명령어와 달라진 것이 없습니다.
