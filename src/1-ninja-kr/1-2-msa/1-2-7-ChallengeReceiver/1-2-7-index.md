...
우리는 맨 처음 Pet Clinic 실습 환경을 구성 할 때 MySQL 파드도 함께 구동시켰기 때문에 현재 PetClinic 의 자바 환경에 업데이트, 생성, 삭제 동작이 호출 될 때 마다 MySQL 에 저장 된 동물과 보호자의 정보를 업데이트 하도록 만들어져 있습니다.

아래와 같이 구동 중임을 확인 해 봅니다

```bash
kubectl get pods | grep petclinic-db

petclinic-db-dfb77856f-ckj8s      1/1     Running   0          69m
```
...
아래 내용을 참고하여 `values.yaml` 파일을 수정 해 봅시다

MySQL 리시버 설정 용 유저정보는 아래와 같습니다.

- `user` : root
- `password` : petclinic
- `database` : petclinic
- `endpoint` : petclinic-db:3306
...
본인 클러스터와 같은 k8s.cluster.name 이 확인된다면 성공입니다
